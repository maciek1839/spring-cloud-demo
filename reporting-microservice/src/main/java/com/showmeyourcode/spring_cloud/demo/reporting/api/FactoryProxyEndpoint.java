...
@Slf4j
@RestController
@Tag(name = "Factory Proxy", description = "Factory Proxy endpoint")
@RequestMapping(EndpointConstant.FACTORY_MICROSERVICE)
public class FactoryProxyEndpoint {
    ...
    public FactoryProxyEndpoint(ObjectMapper objectMapper,
                                @Qualifier(FACTORY_REST_TEMPLATE)
    ...
    @Operation(
            summary = "Get the Factory microservice info",
            responses = {
    ...
    public JsonNode getFactoryActuatorInfo() {
        log.info("Calling the Factory microservice (RestTemplate) ...");
        ...
        return objectMapper.readValue(restTemplate.exchange("/factory/actuator/info",
                HttpMethod.GET,
                httpEntity,
                String.class
